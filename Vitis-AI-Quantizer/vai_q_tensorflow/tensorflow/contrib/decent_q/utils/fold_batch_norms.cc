...
Status GetScaleAndOffsetNodes(const NodeMatch& match,
  ...
  NodeDef add_node;
  add_node.set_op("Add");
  add_node.set_name(batch_norm_node.name() + "/add");
  ...
  inputs_to_rename[batch_norm_node.name()] = add_node.name();
  return Status::OK();
}
...
Status FoldBatchNormsTraining(const GraphDef& input_graph_def,
  ...
  TF_RETURN_IF_ERROR(matcher.GetOpTypeMatches(bn_moving_mean_var_pattern,
                                              &matches, allow_intersection));
    ...
    TF_RETURN_IF_ERROR(ReplaceMatchingOpTypes(
        ...
        [&did_graph_change, &inputs_to_rename, &nodes_to_ignore](
          ...
          if (conv_node.op() == "DepthwiseConv2dNative") {
            ...
            auto weights_flatten = weights.flat<float>();
            auto scaled_weights_flatten = scaled_weights.flat<float>();
            // If the mul-value is only one element, here must fetch the first
            // number.
